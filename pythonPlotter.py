...
Please turn on your X-server first and then hit [enter]"""
    ...
    def plotConfigs(self, xLabel, yLabel, zLabel, threeD, title, numOfPlots, plotCounter, plotType, numData):
        ...
        if config.multipleAxis:
            for i in range(numData - 1):
                self.guest[i].set_ylabel(yLabel[i + 1], size = config.axisLabelSize)
        if threeD: 
        ...
        if not plotType in {'box', 'histogram'} and not config.multipleAxis: # box and hist plots do not have legend. multiple-axis plot has its own way of generating legends (e.g. line #159) 
        ...
        for k in range(len(config.axesOff)):
            ...
            if config.multipleAxis:
                for i in range(numData - 1):
                    self.guest[i].spines[config.axesOff[k]].set_visible(False)
        if not plotType in {'box', 'cdf'}: # function below (?) messes up the ticknames of box plots. To be investigated.
            self.padAndScale(numData)
    ...
    def padAndScale(self, numData):
        ...
        if config.multipleAxis:
            for i in range(numData - 1):
                self.guest[i].tick_params(axis = 'y', labelsize = config.yTickSize)
        ...
        if plotSelect == 'bar':
            ...
            if config.multipleAxis: # multiple-axis enabled
                ...
                if plotPlotSelect[0] == 1: # line plot
                    p[0], = self.host[self.figColCnt, self.figRowCnt].plot(data[colNumX[0]], data[colNumY[0]], self.colors[config.lineColors[0]], label = legendName[0], alpha = config.alpha, linewidth = config.lineWidth[0])  
                    ...
                    p[0], p2[-1], = self.host[self.figColCnt, self.figRowCnt].plot(data[colNumX[0]], data[colNumY[0]], '-x', self.colors[config.lineColors[0]], label = legendName[0], alpha = config.alpha, linewidth = config.lineWidth[0])    
                elif plotPlotSelect[0] == 4: # line plot w/ errorbar
                    p[0], = self.host[self.figColCnt, self.figRowCnt].errorbar(data[colNumX[0]], data[colNumY[0]], yerr = data[colNumE[0]], fmt = config.lineErrPlot_lineStyle, c = self.colors[config.lineColors[1]], label = legendName[0])
                elif plotPlotSelect[0] == 5: # scatter plot w/ errorbar
                    p[0], = self.host[self.figColCnt, self.figRowCnt].errorbar(data[colNumX[0]], data[colNumY[0]], yerr = data[colNumE[0]], fmt = config.scatterErrPlot_lineStyle, c = self.colors[config.lineColors[1]], label = legendName[0])
                else: # line+scatter plot w/ errorbar
                    p[0], p2[-1], = self.host[self.figColCnt, self.figRowCnt].errorbar(data[colNumX[0]], data[colNumY[0]], yerr = data[colNumE[0]], fmt = config.lineScatErrPlot_lineStyle, c = self.colors[config.lineColors[1]], label = legendName[0])
                for i in range(1, numData):
                    ...
                    if plotPlotSelect[0] == 1: # line plot
                        p[i], = self.guest[j].plot(data[colNumX[i]], data[colNumY[i]], self.colors[config.lineColors[i]], label = legendName[i], alpha = config.alpha, linewidth = config.lineWidth[i])  
                    elif plotPlotSelect[0] == 2: # scatter plot
                        ...
                        p2.append(0) #initialize array entry
                        p[i], p2[-1], = self.guest[j].plot(data[colNumX[i]], data[colNumY[i]], '-x', self.colors[config.lineColors[i]], label = legendName[i], alpha = config.alpha, linewidth = config.lineWidth[i])    
                    elif plotPlotSelect[0] == 4: # line plot w/ errorbar
                        p[i], = self.guest[j].errorbar(data[colNumX[i]], data[colNumY[i]], yerr = data[colNumE[i]], fmt = config.lineErrPlot_lineStyle, c = self.colors[config.lineColors[i]], label = legendName[i])
                    elif plotPlotSelect[0] == 5: # scatter plot w/ errorbar
                        p[i], = self.guest[j].errorbar(data[colNumX[i]], data[colNumY[i]], yerr = data[colNumE[i]], fmt = config.scatterErrPlot_lineStyle, c = self.colors[config.lineColors[i]], label = legendName[i])
                    else: # line+scatter plot w/ errorbar
                        p2.append(0) #initialize array entry
                        p[i], p2[-1], = self.guest[j].errorbar(data[colNumX[i]], data[colNumY[i]], yerr = data[colNumE[i]], fmt = config.lineScatErrPlot_lineStyle, c = self.colors[config.lineColors[i]], label = legendName[i])
            ...
            if config.multipleAxis: # multiple-axis enabled
                sns.lineplot(x = data[colNumX[0]], y = data[colNumY[0]], color = self.colors[config.lineColors[0]], label = legendName[0], ax = self.host[self.figColCnt, self.figRowCnt], linewidth = config.lineWidth[0])
                for i in range(1, numData):
                    self.guest.append(0) #initialize array entry
                    j = i - 1
                    if i != numData:
                        self.guest[j] = self.host[self.figColCnt, self.figRowCnt].twinx() # setup 2nd axis based on the first graph
                    sns.lineplot(x = data[colNumX[i]], y = data[colNumY[i]], color = self.colors[config.lineColors[i % len(config.lineColors)]], label = legendName[i], ax = self.guest[j], linewidth = config.lineWidth[i])
                    self.guest[j].set_ylim(min(data[colNumY[i]]) - config.yLimThreshold, max(data[colNumY[i]]) + config.yLimThreshold)
                self.axisColoring(numData)
            else:
                for i in range(numData):
